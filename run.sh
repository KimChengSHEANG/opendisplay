...
#
# Always replaces a running instance so `make run` picks up a fresh binary
# (macOS `open` alone reuses the already-running process).
set -e
cd "$(dirname "$0")"
...
# Quit any live instance; ignore "no matching processes" so a cold start is fine.
killall OpenDisplay 2>/dev/null || true
# Brief pause so WindowServer releases the previous virtual-display serial
# before the new process recreates it.
sleep 0.3
